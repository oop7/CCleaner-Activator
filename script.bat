...
set "DEFAULT_DEST_DIR=C:\Program Files\CCleaner"
...
echo %GREEN%2. Hosts blocker%RESET%
...
if "%choice%"=="1" (
    ...
    copy "%TEMP%\decoded_file.txt" "%DEFAULT_DEST_DIR%\branding.dll" >nul
    copy "%TEMP%\decoded_2_file.txt" "%DEFAULT_DEST_DIR%\CCleaner.dat" >nul
    if %errorlevel% neq 0 (
        echo %RED%Failed to copy the files.%RESET%
    )
    pause
)else if "%choice%"=="2" (
    echo Verifying source file...

    if not exist "%TEMP%\decoded_3_file.txt" (
        echo %RED%Source file "decoded_3_file.txt" not found. Please verify.%RESET%
        pause
        exit /b
    )

    echo Source file exists.
    echo CCleaner hosts blocker...
    rename "%TEMP%\decoded_3_file.txt" "decoded_3_file.bat"
    call "%TEMP%\decoded_3_file.bat"
    if %errorlevel% neq 0 (
        echo %RED%Failed to install the file.%RESET%
    )
    pause
) else if "%choice%"=="3" (
    echo Exiting...
